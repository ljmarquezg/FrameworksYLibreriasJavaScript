...
var gameOver = false //Continuar jugando
...
$(function(){
  ...
  /*** Extra - Icono para habilitar mensajes de puntiación ***/
  ...
  /*** Acciones del boton inicio***/
  $('.btn-reinicio').on('click',function(){
    if(reiniciar == 0){ //Verificar que no exista una partida en curso
      iniciarCronometro(interval); //Iniciar cronómetro
      matchElements(); //Verificar si existen elementos consecutivos en el tablero.
      $(this).html("Reiniciar"); //Cambiar el texto del botón a "Reiniciar".
      reiniciar++ //Indicar que se ha iniciado una partida.
    }else{
  ...
  function colorTitulo(){
    $('.main-titulo').toggleClass('main-titulo-inverse') //Agrega / Remueve la clase main-titulo-inverse para cambiar el color del título
  }
  ...
  function deshabilitarMovimientos(){
    ...
    console.log("Operación en curso. Movimiento Deshabilitado") //Mostrar mensaje en cosola
  }
  ...
  function habilitarMovimientos(){
    ...
    console.log("Movimiento Habilitado") //Mostrar mensaje en cosola
  }
  ...
  function validarMovimiento(top,left){
    ...
    if((left == desplazamientoHorizontal+"px" || left == "-"+desplazamientoHorizontal+"px") && (top == "0px")) {//Validar movimientos laterales Limitando movimiento diagonal
      ...
      horizontal = true;
      console.log("Desplanzando hacia la " +desplazamiento); //Mostrar mensaje en cosola
      ...
      vertical = true;
      console.log("Dezplazando hacia la "+ desplazamiento); //Mostrar mensaje en cosola
    }else{
      horizontal = false;
      vertical = false;
      console.log("Movimiento NO Premitido"); //Mostrar mensaje en cosola
    }
  ...
  function desplazarElemento(desplazamiento){
    ...
    /*** Intercambiar elementos desplazados ***/
  ...
  /*** Verificar si 3 o más existen elementos consecutivos ***/
  function matchElements(){
      ...
      if(matchDulcesConsecutivos.length > 0){ //Verificar si existen dulces consecutivos
        ...
        if(gameOver == false){ //Realizar accciones mientras el juego se mantenga activo.
          habilitarMovimientos(); //Rehabilitar movimientos
        }else{
          /*** El juego continuará realizando movimientos hasta que no hayan mas dulces consevutivos.***/
          deshabilitarMovimientos() //Juego terminado - deshabilitar movimientos
          $('.panel-tablero, .time').hide('slow',"swing"); //Esconder el tablero con efecto animado
          $('.panel-tablero').before('<h1 class="titulo-over over-container">Juego Terminado</h1>'); //Mostrar el mensaje Juego Terminado
          $('.panel-score').animate({
            width : "100%" //Haecr que los cuadros de panel cubran toda la posicion horizontal de la pantalla
          },"slow")
          console.log("Juego Finalizado") //Mostrar mensaje en cosola
        }
      }
    }
  ...
  function efectoMatch(){
    ...
    if(contadorEfecto < totalContadorEfecto){
      ...
      if(habilitarMensajes == true){
        switch (true){
          case (puntos == 30):
          $('#mensaje').text("Bien!").show("fade", tiempoEfecto); //Cambiar el mensaje de puntuación de acuerdo a los puntos obtenidos
          break;

          case (puntos > 30 && puntos <= 90):
          $('#mensaje').text("Genial!").show("fade", tiempoEfecto); //Cambiar el mensaje de puntuación de acuerdo a los puntos obtenidos
          break;

          case (puntos> 90 && puntos <=120):
          $('#mensaje').text("Increíble!").show("fade", tiempoEfecto); //Cambiar el mensaje de puntuación de acuerdo a los puntos obtenidos
          break;

          case (puntos > 120):
          $('#mensaje').text("Soprendente!").show("fade", tiempoEfecto); //Cambiar el mensaje de puntuación de acuerdo a los puntos obtenidos
          break
      ...
      if(habilitarMensajes == true){
        $('#mensaje').hide("fade", tiempoEfecto/4); //Ocultar el mensaje de puntuación
      }
...
function iniciarCronometro(interval) {
  ...
  if(secs !== -1){
    ...
    gameOver = true; //Finalizar la partida actual
  }
}
